...
struct pageData parsePageData(unsigned char* buffer, int channels){ //will parse the Ogg page data
  ...
  char sig[4];
  char preSkip[2];
  char absoluteGranulePosition[8];
  char streamSerialNumber[4];
  char pageSequenceNumber[4];
  char pageChecksum[4];
  char pageSegements[1];
